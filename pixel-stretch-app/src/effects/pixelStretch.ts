...
function pixelsToCanvas(
  ...
  out.width = width
  out.height = height
  // slice() guarantees an ArrayBuffer-backed copy (required by ImageData)
  out.getContext('2d')!.putImageData(new ImageData(outPixels.slice(), width, height), 0, 0)
  return out
}

function getSourcePixels(sourceCanvas: HTMLCanvasElement): {
  width: number
  height: number
...
export function radialStretchPixels(
  ...
  if (sx < 0 || sx >= width || sy < 0 || sy >= height) return outPixels
...
export function radialStretch(
  ...
  const { width, height, data } = getSourcePixels(sourceCanvas)
  const outPixels = radialStretchPixels(data, width, height, centerX, centerY, stretchH, stretchV, blendMode, easing)
  return pixelsToCanvas(outPixels, width, height)
}

/* ============================ ROW ============================ */

export function rowStretchPixels(
  srcPixels: Uint8ClampedArray,
  width: number,
  height: number,
  row: number,
  ...
  if (r < 0 || r >= height) return outPixels
...
export function rowStretch(
  ...
  const { width, height, data } = getSourcePixels(sourceCanvas)
  const outPixels = rowStretchPixels(data, width, height, row, stretchUp, stretchDown, blendMode, easing)
  return pixelsToCanvas(outPixels, width, height)
}

/* ============================ COLUMN ============================ */

export function columnStretchPixels(
  srcPixels: Uint8ClampedArray,
  width: number,
  height: number,
  col: number,
  ...
  if (c < 0 || c >= width) return outPixels
...
export function columnStretch(
  ...
  const { width, height, data } = getSourcePixels(sourceCanvas)
  const outPixels = columnStretchPixels(data, width, height, col, stretchLeft, stretchRight, blendMode, easing)
  return pixelsToCanvas(outPixels, width, height)
}

/* ============================ RADIAL FULL ============================ */

export function radialStretchFullPixels(
  srcPixels: Uint8ClampedArray,
  width: number,
  height: number,
  centerX: number,
  ...
  if (sx < 0 || sx >= width || sy < 0 || sy >= height || maxRadius < 2) return outPixels
...
export function radialStretchFull(
  ...
  const { width, height, data } = getSourcePixels(sourceCanvas)
  const outPixels = radialStretchFullPixels(data, width, height, centerX, centerY, maxRadius, blendMode, easing)
  return pixelsToCanvas(outPixels, width, height)
}

/* ============================ MIRROR ============================ */

export function mirrorStretchPixels(
  srcPixels: Uint8ClampedArray,
  width: number,
  height: number,
  lineType: 'row' | 'column',
  ...
  if (lineType === 'row') {
    if (pos < 0 || pos >= height) return outPixels
    for (let x = 0; x < width; x++) {
    ...
    if (pos < 0 || pos >= width) return outPixels
    for (let y = 0; y < height; y++) {
      for (let d = 1; d <= mirrorDist; d++) {
        const left = pos - d
        const right = pos + d
...
export function mirrorStretch(
  ...
  const { width, height, data } = getSourcePixels(sourceCanvas)
  const outPixels = mirrorStretchPixels(data, width, height, lineType, linePos, mirrorDist, blendMode)
  return pixelsToCanvas(outPixels, width, height)
}
...
export function twirlEffect(
  ...
  const { width, height, data } = getSourcePixels(sourceCanvas)
  const outPixels = twirlEffectPixels(data, width, height, centerX, centerY, intensity, blendMode, radius)
  return pixelsToCanvas(outPixels, width, height)
}

/* ============================ SELECTION WARP ============================ */

export function selectionWarpPixels(
  srcPixels: Uint8ClampedArray,
  width: number,
  height: number,
  selX: number,
  ...
  for (let py = sy; py < sy + sh; py++) {
    for (let px = sx; px < sx + sw; px++) {
      ...
      outPixels[dstI + 1] = srcPixels[srcI + 1]
      outPixels[dstI + 2] = srcPixels[srcI + 2]
      outPixels[dstI + 3] = computeAlpha(px, py, rawAlpha, blendMode)
    }
  }
...
export function selectionWarp(
  ...
  const { width, height, data } = getSourcePixels(sourceCanvas)
  const outPixels = selectionWarpPixels(data, width, height, selX, selY, selW, selH, dragX, dragY, blendMode)
  return pixelsToCanvas(outPixels, width, height)
}
