...
import { runEffectToCanvas, isWorkerUnavailable } from '../effects/workerPool'
...
interface DragState {
  ...
  startLayerX?: number
  startLayerY?: number
  /** Touch gesture that might still resolve to a tap (move the source line). */
  maybeTap?: boolean
}

/** Smallest drag (in canvas px) that counts as a real gesture on touch. */
const TAP_SLOP = 4
...
export function Canvas() {
  ...
  const handlePointerDown = useCallback(
    (e: React.PointerEvent) => {
      // Ignore extra fingers while a gesture is already active (pinch zoom)
      ...
      if (tool === 'stretch-row' || tool === 'stretch-column') {
        ...
        // Touch/pen: a drag over an existing source line stretches directly
        // (no Shift key on iPhone). Desktop keeps the Shift+drag flow.
        const isTouch = e.pointerType === 'touch' || e.pointerType === 'pen'
        const canStretch = !!(sourceLine && (e.shiftKey || isTouch))

        if (canStretch) {
          // Stretch from the existing source line
          dragRef.current = {
          ...
          setStretchPreview({ type, sourcePos: sourceLine!.position, currentPos: position })
        } else {
          // Simple tap = set/update the source line
  ...
  const applyStretch = useCallback(async () => {
    ...
    // Touch tap on a source line = move the line instead of stretching
    if (drag.mode === 'stretch' && drag.maybeTap) {
      const type = drag.sourceType!
      const position = type === 'row' ? Math.floor(drag.startY) : Math.floor(drag.startX)
      useLayerStore.getState().setSourceLine({ type, position })
      useLayerStore.getState().setStretchPreview(null)
      dragRef.current = null
    ...
    if (drag.mode === 'radial') {
      ...
      if (stretchH > 2 || stretchV > 2) {
        const cx = drag.startX - lx
        const cy = drag.startY - ly
        plan = {
          fn: 'radialStretch',
    ...
    // Clear transient preview state before running the (possibly async) effect
    previewCanvasRef.current = null
    lastPreviewParams.current = ''
    dragRef.current = null
    setStretchPreview(null)
    ...
    try {
      ...
      if (!isWorkerUnavailable()) {
        try {
          result = await runEffectToCanvas(plan.fn, plan.canvas, ...plan.args)
        } catch {
          // Worker failure (timeout, restricted env) → synchronous fallback
          result = plan.fallback()
        }
      } else {
        result = plan.fallback()
      }
      addLayer(result, plan.name, compositeOp)
    } catch (err) {
  ...
  const handlePointerMove = useCallback(
    (e: React.PointerEvent) => {
      ...
      if (drag.mode === 'stretch' && drag.sourceType) {
        // On touch, a stretch gesture only starts after real movement,
        // otherwise the gesture is a tap that moves the source line.
        if (drag.maybeTap) {
          const dx = pos.x - drag.startX
          const dy = pos.y - drag.startY
    ...
    (e: React.PointerEvent) => {
      ...
      if (ov) {
        try {
          ...
          // pointer was never captured (e.g. second finger) — ignore
        }
      }
      void applyStretch()
    },
  ...
  // Pinch-to-zoom + double-tap-to-zoom (touch)
  useEffect(() => {
    ...
    const handleTouchEnd = (e: TouchEvent) => {
      ...
      if (now - lastTapTime < 300) {
        lastTapTime = 0
        const state = useLayerStore.getState()
        // Double-tap zooms only in view tools, to never conflict with edits
        if (state.tool === 'move' || state.tool === 'zoom') {
          state.setZoom(state.zoom > 1.5 ? 1 : 2)
        }
      } else {
        lastTapTime = now
      }
