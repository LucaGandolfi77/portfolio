...
async function interactNpc(key) {
  ...
  flags[cfg.flag] = true;
  if (gameState) {
    gameState.flags[cfg.flag] = true;
    if (!gameState.npcMet.includes(cfg.flag)) gameState.npcMet.push(cfg.flag);
  }
  scene.removeFigure(key);
  AudioSys.bellToll(330);
  setExploring(true);
  saveGame();
}
...
function beginBattle(key) {
  ...
  AudioSys.playTheme(ZONES[zoneIndex]?.theme || 'soglia');
  combat = createCombat({
    scene,
    enemyKey: key,
    bonus: flags,
...
async function onWin() {
  ...
  await sleep(800);
  combat = null;
  if (gameState && !gameState.bossesDefeated.includes(currentEnemyKey)) {
    gameState.bossesDefeated.push(currentEnemyKey);
  }
  saveGame();
  ...
  if (step) {
    ...
    if (gameState) gameState.choicesMade.finale = key;
    const ending = ENDINGS[key];
    AudioSys.bellToll(165);
    showEnding(ending);
  }
}
...
function initGameState(overrides) {
  gameState = SaveSys.newGame(overrides);
  playStartTime = Date.now();
  // Restore flags from NPC interactions
  Object.keys(NPC_DIALOGUES).forEach(key => {
    const cfg = NPC_DIALOGUES[key];
    if (gameState.flags[cfg.flag]) {
      gameState.npcMet.push(cfg.flag);
    }
  });
}
...
function saveGame() {
  if (!gameState) return;
  gameState.totalPlayTime += Date.now() - playStartTime;
  playStartTime = Date.now();
  gameState.zoneIndex = zoneIndex;
  gameState.flags = Object.assign({}, flags);
  SaveSys.autoSave(gameState);
}
