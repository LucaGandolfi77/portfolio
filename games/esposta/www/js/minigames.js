...
window.Minigames = {
  run(id, area, onDone) {
    window.Analytics && window.Analytics.trackEvent('minigame_started', { minigameId: id });
    const fn = {
  ...
  _showResult(area, score, total, onDone, msg) {
    ...
    area.innerHTML = `
      <div class="mg-card fade-in" style="text-align:center">
        ...
        <div class="mg-title">${this._t('mg_completed')}</div>
        <div class="mg-score">${score}/${total} — ${pct}%</div>
        <div style="font-size:12px;color:var(--dim);margin:8px 0;font-style:italic">${msg||''}</div>
        <button class="btn primary" id="mg-continue">${this._t('mg_continue')}</button>
      </div>`;
    document.getElementById('mg-continue').onclick = () => {
      window.Analytics && window.Analytics.trackEvent('minigame_completed', { score, total });
      onDone(score, total);
    };
  },
  ...
  _esposizione(area, onDone) {
    ...
    const render = () => {
      ...
      const l = this._lang();
      const noiseKey = iso > 1600 ? 'mg_esposizione_noise_high' : iso > 400 ? 'mg_esposizione_noise_mod' : 'mg_esposizione_noise_clean';
      const depthKey = ap < 2.8 ? 'mg_esposizione_depth_extreme' : ap < 5.6 ? 'mg_esposizione_depth_medium' : 'mg_esposizione_depth_all';
      const freezeKey = sh > 500 ? 'mg_esposizione_freeze_frozen' : sh > 60 ? 'mg_esposizione_freeze_still' : 'mg_esposizione_freeze_trail';
      document.getElementById('exp-preview').style.filter = `brightness(${brightness}) saturate(0.3) sepia(${0.6 + brightness * 0.3})`;
      document.getElementById('exp-label').textContent = `${this._t(noiseKey)} · ${this._t(depthKey)} · ${this._t(freezeKey)}`;
      document.getElementById('exp-ev').textContent = `EV ${evCalc.toFixed(1)}`;
    };
    area.innerHTML = `
      <div class="mg-title">${this._t('mg_esposizione_title')}</div>
      <div class="mg-sub">${this._t('mg_esposizione_sub')}</div>
      <div class="mg-card">
        <div class="exp-preview" id="exp-preview">🎞️</div>
        <div class="exp-label" id="exp-label">${this._t('mg_esposizione_noise_clean')} · ${this._t('mg_esposizione_depth_medium')} · ${this._t('mg_esposizione_freeze_still')}</div>
        <div class="mg-score">EV target: 4.2 | EV ${this._lang()==='en'?'you':'tuo'}: <span id="exp-ev">3.5</span></div>
      </div>
      <div class="mg-card">
        <div class="slider-wrap"><label>${this._t('exp_slider_iso')}</label><input type="range" id="sl-iso" min="50" max="6400" step="50" value="200"><span class="val" id="v-iso">200</span></div>
        <div class="slider-wrap"><label>${this._t('exp_slider_aperture')}</label><input type="range" id="sl-ap" min="14" max="220" step="1" value="56"><span class="val" id="v-ap">f/5.6</span></div>
        <div class="slider-wrap"><label>${this._t('exp_slider_shutter')}</label><input type="range" id="sl-sh" min="1" max="4000" step="1" value="125"><span class="val" id="v-sh">1/125s</span></div>
      </div>
      <button class="btn primary" id="exp-btn">${this._t('mg_esposizione_btn')}</button>`;
    ...
    document.getElementById('exp-btn').onclick = () => {
      ...
      this._showResult(area, score, 10, onDone, score >= 8 ? this._t('mg_esposizione_perfect') : this._t('mg_esposizione_retry'));
    };
  },
  ...
  _composizione(area, onDone) {
    const targets = [
      { x: 33, y: 33, name: this._t('mg_composizione_target_left_top') },
      { x: 67, y: 33, name: this._t('mg_composizione_target_right_top') },
      { x: 33, y: 67, name: this._t('mg_composizione_target_left_bottom') },
      { x: 67, y: 67, name: this._t('mg_composizione_target_right_bottom') }
    ];
    const subjects = ['👤','🐱','🌳','☕'];
    let round = 0, score = 0, total = 4;
    const render = () => {
      if (round >= total) { this._showResult(area, score, total, onDone, score >= 3 ? this._t('mg_composizione_natural') : this._t('mg_composizione_hint')); return; }
      const t = targets[round];
      area.innerHTML = `
        <div class="mg-title">${this._t('mg_composizione_title')}</div>
        <div class="mg-sub">${this._t('mg_composizione_sub')} (${round+1}/${total})</div>
        <div class="mg-card">
  ...
  _luce(area, onDone) {
    const l = this._lang();
    const scenarios = (l === 'en' && window.MINIGAMETranslations?.en?.light_scenarios) ? window.MINIGAMETranslations.en.light_scenarios : [
    ...
    let idx = 0, score = 0;
    const render = () => {
      if (idx >= scenarios.length) { this._showResult(area, score, scenarios.length, onDone, score >= 4 ? this._t('mg_luce_eye') : this._t('mg_luce_practice')); return; }
      const s = scenarios[idx];
      const shuff = this._shuffle(s.opts);
      area.innerHTML = `
        <div class="mg-title">${this._t('mg_luce_title')}</div>
        <div class="mg-sub">${this._t('mg_luce_sub')} (${idx+1}/${scenarios.length})</div>
  ...
  _momento(area, onDone) {
    ...
    const startRound = () => {
      if (round >= total) { this._showResult(area, score, total, onDone, score >= 4 ? this._t('mg_momento_caught') : this._t('mg_momento_practice')); return; }
      zonePos = Math.random() * 70 + 10;
      zoneW = 10 + Math.random() * 8;
      cursorPos = 0;
      const speed = 1.5 + round * 0.3;
      area.innerHTML = `
        <div class="mg-title">${this._t('mg_momento_title')}</div>
        <div class="mg-sub">${this._t('mg_momento_sub')} (${round+1}/${total})</div>
        <div class="mg-card">
        ...
        <button class="btn primary" id="snap-btn">${this._t('mg_momento_btn')}</button>`;
  ...
  _algoritmo(area, onDone) {
    const l = this._lang();
    const factors = (l === 'en' && window.MINIGAMETranslations?.en?.algo_factors) ? window.MINIGAMETranslations.en.algo_factors : [
    ...
    let idx = 0, score = 0;
    const render = () => {
      if (idx >= factors.length) { this._showResult(area, score, factors.length, onDone, score >= 4 ? this._t('mg_algoritmo_loves') : this._t('mg_algoritmo_buries')); return; }
      const f = factors[idx];
      const shuff = this._shuffle(f.opts);
      area.innerHTML = `
        <div class="mg-title">${this._t('mg_algoritmo_title')}</div>
        <div class="mg-sub">${this._t('mg_algoritmo_sub')} (${idx+1}/${factors.length})</div>
  ...
  _contenuto(area, onDone) {
    const l = this._lang();
    const hooks = (l === 'en' && window.MINIGAMETranslations?.en?.content_hooks) ? window.MINIGAMETranslations.en.content_hooks : [
    ...
    let idx = 0, score = 0;
    const render = () => {
      if (idx >= hooks.length) { this._showResult(area, score, hooks.length, onDone, score >= 3 ? this._t('mg_contenuto_ninja') : this._t('mg_contenuto_practice')); return; }
      const h = hooks[idx];
      const shuff = this._shuffle(h.opts);
      area.innerHTML = `
        <div class="mg-title">${this._t('mg_contenuto_title')}</div>
        <div class="mg-sub">${this._t('mg_contenuto_sub')} (${idx+1}/${hooks.length})</div>
  ...
  _community(area, onDone) {
    const l = this._lang();
    const crises = (l === 'en' && window.MINIGAMETranslations?.en?.community_crises) ? window.MINIGAMETranslations.en.community_crises : [
    ...
    let idx = 0, score = 0;
    const render = () => {
      if (idx >= crises.length) { this._showResult(area, score, crises.length, onDone, score >= 2 ? this._t('mg_community_expert') : this._t('mg_community_practice')); return; }
      const c = crises[idx];
      const shuff = this._shuffle(c.opts);
      area.innerHTML = `
        <div class="mg-title">${this._t('mg_community_title')}</div>
        <div class="mg-sub">${this._t('mg_community_sub')} (${idx+1}/${crises.length})</div>
  ...
  _analytics(area, onDone) {
    const l = this._lang();
    const questions = (l === 'en' && window.MINIGAMETranslations?.en?.analytics_questions) ? window.MINIGAMETranslations.en.analytics_questions : [
    ...
    let idx = 0, score = 0;
    const render = () => {
      if (idx >= questions.length) { this._showResult(area, score, questions.length, onDone, score >= 3 ? this._t('mg_analytics_expert') : this._t('mg_analytics_practice')); return; }
      const q = questions[idx];
      const shuff = this._shuffle(q.opts);
      area.innerHTML = `
        <div class="mg-title">${this._t('mg_analytics_title')}</div>
        <div class="mg-sub">${this._t('mg_analytics_sub')} (${idx+1}/${questions.length})</div>
  ...
  _marketingMix(area, onDone) {
    const l = this._lang();
    const ps = (l === 'en' && window.MINIGAMETranslations?.en?.marketing_ps) ? window.MINIGAMETranslations.en.marketing_ps : [
    ...
    let idx = 0, score = 0;
    const render = () => {
      if (idx >= ps.length) { this._showResult(area, score, ps.length, onDone, score >= 5 ? this._t('mg_marketing_master') : this._t('mg_marketing_practice')); return; }
      const p = ps[idx];
      const shuff = this._shuffle(p.opts);
      area.innerHTML = `
        <div class="mg-title">${this._t('mg_marketing_title')}</div>
        <div class="mg-sub">${this._t('mg_marketing_sub')} (${idx+1}/${ps.length})</div>
  ...
  _funnel(area, onDone) {
    const l = this._lang();
    const stages = [this._t('funnel_tofu'), this._t('funnel_mofu'), this._t('funnel_bofu'), this._t('funnel_retention')];
    const contents = (l === 'en' && window.MINIGAMETranslations?.en?.funnel_contents) ? window.MINIGAMETranslations.en.funnel_contents : [
    ...
    let idx = 0, score = 0;
    const render = () => {
      if (idx >= stages.length) { this._showResult(area, score, stages.length, onDone, score >= 3 ? this._t('mg_funnel_master') : this._t('mg_funnel_practice')); return; }
      ...
      area.innerHTML = `
        <div class="mg-title">${this._t('mg_funnel_title')}</div>
        <div class="mg-sub">${this._t('mg_funnel_sub')} <b>${stages[idx]}</b>? ${this._lang()==='en'?'Choose 3':'Scegli 3'}. (${idx+1}/${stages.length})</div>
        <div class="mg-grid" style="grid-template-columns:1fr">${opts.map((o,i) =>
          `<button class="mg-btn" data-opt="${i}">${o}</button>`
        ).join('')}</div>
        <button class="btn primary" id="funnel-ok">${this._t('mg_funnel_confirm')}</button>`;
  ...
  _brand(area, onDone) {
    const l = this._lang();
    const brandNames = (l === 'en' && window.MINIGAMETranslations?.en?.brand_positions) ? window.MINIGAMETranslations.en.brand_positions : [
    ...
    let idx = 0, score = 0;
    const render = () => {
      if (idx >= brandNames.length) { this._showResult(area, score, brandNames.length, onDone, score >= 3 ? this._t('mg_brand_strategist') : this._t('mg_brand_practice')); return; }
      const b = brandNames[idx];
      const shuff = this._shuffle(b.opts);
      area.innerHTML = `
        <div class="mg-title">${this._t('mg_brand_title')}</div>
        <div class="mg-sub">${this._t('mg_brand_sub')} (${idx+1}/${brandNames.length})</div>
  ...
  _persuasione(area, onDone) {
    const l = this._lang();
    const copies = (l === 'en' && window.MINIGAMETranslations?.en?.persuasion_copies) ? window.MINIGAMETranslations.en.persuasion_copies : [
    ...
    let idx = 0, score = 0;
    const render = () => {
      if (idx >= copies.length) { this._showResult(area, score, copies.length, onDone, score >= 3 ? this._t('mg_persuasione_persuasive') : this._t('mg_persuasione_practice')); return; }
      const c = copies[idx];
      const shuff = this._shuffle(c.opts);
      area.innerHTML = `
        <div class="mg-title">${this._t('mg_persuasione_title')}</div>
        <div class="mg-sub">${this._t('mg_persuasione_sub')} (${idx+1}/${copies.length})</div>
  ...
  _esame(area, onDone) {
    window.Analytics && window.Analytics.trackEvent('exam_started', { totalQuestions: 18 });
    const l = this._lang();
    const qs = (l === 'en' && window.MINIGAMETranslations?.en?.exam_questions) ? window.MINIGAMETranslations.en.exam_questions : [
    ...
    const render = () => {
      if (idx >= qs.length) {
        ...
        area.innerHTML = `
          <div class="mg-card fade-in" style="text-align:center">
            ...
            <div class="mg-title">${passed ? this._t('mg_esame_passed') : this._t('mg_esame_failed')}</div>
            <div class="mg-score">${score}/${qs.length} — ${pct}%</div>
            <div style="font-size:12px;color:var(--dim);margin:8px 0;font-style:italic">${passed ? this._t('mg_esame_ready') : this._t('mg_esame_retry_msg')}</div>
            <button class="btn primary" id="esame-done">${passed ? this._t('mg_esame_enter_agency') : this._t('mg_esame_retry_btn')}</button>
          </div>`;
        document.getElementById('esame-done').onclick = () => {
          window.Analytics && window.Analytics.trackEvent('exam_completed', { score, totalQuestions: qs.length, passed: pct >= 70 });
      ...
      const q = qs[idx];
      area.innerHTML = `
        <div class="mg-title">${this._t('mg_esame_title')}</div>
