...
(function () {
  ...
  const keyEls = {};       // lettera -> elemento tasto
  const keyStates = {};    // lettera -> stato noto migliore ('G'|'Y'|'B')
  const STATE_RANK = { B: 0, Y: 1, G: 2 };
  ...
  function confetti() {
    ...
    for (let i = 0; i < 70; i++) {
      if (i % 6 === 5) {
        ...
        s.textContent = CONFETTI_EMOJI[(i / 6 | 0) % CONFETTI_EMOJI.length];
  ...
  /* Stato noto migliore per lettera: G batte Y batte B.
     Es. prima vedo B poi Y -> la lettera "sale" a gialla. */
  function bestState(a, b) { return STATE_RANK[a] >= STATE_RANK[b] ? a : b; }
  function updateKeyStates(guess, fb) {
    for (let i = 0; i < guess.length; i++) {
      const l = guess[i];
      if (!LETTERS.includes(l)) continue;
      keyStates[l] = keyStates[l] ? bestState(fb[i], keyStates[l]) : fb[i];
      const el = keyEls[l];
      if (el) el.className = 'key k-' + keyStates[l].toLowerCase();
    }
  }
  ...
  function pressKey(k) {
    ...
    if (k === 'BACK') { state.current = state.current.slice(0, -1); }
      ...
      if (state.current.length < len) state.current += k;
      vibrate(4);
    }
    const el = keyEls[k];
  ...
  function setLift(on) {
    ...
    if (row) row.classList.toggle('lift', !!on);
  }
  ...
  function renderInput() {
    ...
    document.querySelectorAll('.row').forEach((r) => r.classList.remove('lift'));
    if (state.running && !state.locked) row.classList.add('lift');
  }
  ...
  function revealRow(guess, feedback) {
    ...
    for (let c = 0; c < len; c++) {
      ...
      t.style.animationDelay = (c * 45) + 'ms';
    }
  }
  ...
  function renderScoreStrip() {
    ...
    state.players.forEach((p) => {
      const chip = document.createElement('div');
      chip.className = 'chip';
      chip.innerHTML = p.name + ' <span class="sips' + (p.sips > 0 ? ' hot' : '') + '">🍺 ' + p.sips + '</span>';
      strip.appendChild(chip);
    });
  }

  function showDrink(text, sips, sev) {
    ...
    banner.classList.remove('sev-maker', 'sev-guesser', 'shake');
    if (sev === 'maker') banner.classList.add('sev-maker');
    else if (sev === 'guesser') banner.classList.add('sev-guesser');
    banner.hidden = false;
  ...
  function submitGuess() {
    ...
    if (!text) text = '\u{1F60C} Giro pulito: nessuno beve';
    const sev = drinks.guesserSips ? 'guesser' : drinks.makerSips ? 'maker' : null;
    showDrink(text, drinks.guesserSips + drinks.makerSips, sev);
    renderScoreStrip();
    ...
    if (won) {
      const row = document.querySelector('.row[data-row="' + (state.attempt - 1) + '"]');
      if (row) row.classList.add('win-row');
      endGame(true);
      return;
    }
