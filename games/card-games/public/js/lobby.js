...
function connectSocket(nickname) {
  ...
  waitForTransport(function () {
    if (typeof io !== 'undefined' && !window.__noSocketIO) {
      bindRealSocket(nickname);
    } else {
      bindLocalSocket(nickname);
    }
  });
}
...
function waitForTransport(cb) {
  ...
  (function check() {
    if (typeof io !== 'undefined' || window.__noSocketIO || Date.now() - started > 2000) cb();
    else setTimeout(check, 50);
  })();
}

function bindSocketEvents(s) {
  s.on('roomListUpdate', (rooms) => renderRooms(rooms));

  s.on('roomUpdate', (room) => {
    if (room.code === currentRoomCode) renderLobby(room);
  });

  s.on('chatMessage', ({ from, text }) => {
  ...
  s.on('gameUpdate', (state) => {
    window.gameState = state;
    renderGame(state);
  });
}

function enterHome(nickname, id) {
  window.playerId = id;
...
function bindRealSocket(nickname) {
  socket = io({ query: { nickname } });
  window.socket = socket;

  socket.on('connect', () => enterHome(nickname, socket.id));
...
async function bindLocalSocket(nickname) {
  if (!window.LocalCG) {
    $('login-error').textContent = 'Impossibile avviare la modalità vs Bot';
    show('login');
    return;
  }
  window.LocalCG.boot((err, cg) => {
    ...
    window.__cg = cg;
    // In modalità statica le stanze remote non esistono
    $('rooms-list').style.display = 'none';
    $('room-count').parentElement.style.display = 'none';
    $('no-rooms-msg').style.display = 'none';

    socket = cg.connect(nickname);
    window.socket = socket;
    bindSocketEvents(socket);
    enterHome(nickname, socket.id);
  });
}
...
function loadHome() {
  if (window.__cg) {
    renderGames(window.__cg.api.listGames());
    renderRooms(window.__cg.api.listRooms());
    return;
  }
...
function renderGames(games) {
  ...
  grid.innerHTML = '';
  const icons = {
    scopa: '🃏', briscola: '♠️', blackjack: '🂡', settemezzo: '7️⃣', tressette: '🎋',
    poker: '♣️', uno: '🎯', explodingkittens: '💥', skullking: '☠️', themind: '🧠',
    thiryone: '🀄', ramino: '📋', scala40: '🔟', memory: '🧩', monopolydeal: '🏠', odin: '⚔️'
  };
  const comingSoon = [];
