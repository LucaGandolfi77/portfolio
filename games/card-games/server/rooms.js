...
// Avanza il giro se la mano è finita. Gestisce entrambe le convenzioni dei giochi:
// 'roundEnd' (scopa, briscola, ...) e 'handOver' (odin). Ogni motore si tutela da sé.
function maybeAdvanceRound(room) {
  const gs = room.gameState;
  if (!gs) return;
  if ((gs.phase === 'roundEnd' || gs.phase === 'handOver') && room.game.nextRound) {
    try { room.game.nextRound(gs); } catch (e) { /* fase non compatibile per questo gioco */ }
  }
}
