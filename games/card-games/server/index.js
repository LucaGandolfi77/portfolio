...
io.on('connection', (socket) => {
  ...
  socket.on('startGame', ({ options } = {}, cb) => {
    ...
    for (const pid of room.gameState.playerOrder) {
      ...
      st.gameType = (room.game.meta ? room.game.meta.id : room.game.id);
  ...
  socket.on('playerAction', ({ action }, cb) => {
    ...
    if (result) {
      if (room.gameState.phase === 'roundEnd' || room.gameState.phase === 'handOver') {
        try { room.game.nextRound && room.game.nextRound(room.gameState); } catch (e) {}
      }
