...
class OllamaBookWriterGUI:
    ...
    def _build_ui(self):
        ...
        # Export button (parity with web UI)
        self.btn_export = ttk.Button(btns, text=self.LOCALES[self.ui_language].get('export','Export Book (.txt)'), command=self.export_book)
    ...
    def export_book(self):
        ...
        # Prefer existing output file if available
        out_text = None
        try:
            if b.output_file and Path(b.output_file).exists():
                out_text = Path(b.output_file).read_text(encoding="utf-8")
        except Exception:
            out_text = None

        if out_text is None:
            # Build a best-effort export from in-memory data
            parts = []
            parts.append(f"BOOK TITLE: {b.title}")
            parts.append(f"GENRE: {b.genre}")
            ...
            parts.append("=" * 80)
            parts.append("")
            # Chapters: we may not have full chapter text in memory (written to output_file incrementally)
            if getattr(b, 'chapter_titles', None):
                for i, title in enumerate(b.chapter_titles, start=1):
                    parts.append(f"CHAPTER {i}: {title}")
                    parts.append("-" * 80)
                    parts.append("<Full chapter text not stored in memory; check output file if available>")
                    parts.append("")
                    parts.append("[SUMMARY MEMORY]")
                    parts.append(b.chapter_summaries[i-1] if i-1 < len(b.chapter_summaries) else "")
                    parts.append("")
                    parts.append("=" * 80)
                    parts.append("")
            out_text = "\n".join(parts)

        safe = "".join(c if c.isalnum() or c in (" ", "-", "_") else "_" for c in b.title).strip().replace(" ", "_")
        default_name = (safe or "book") + ".txt"
        path = filedialog.asksaveasfilename(defaultextension=".txt", initialfile=default_name, filetypes=[("Text files", "*.txt"), ("All files", "*.*")])
        if path:
            try:
                Path(path).write_text(out_text, encoding="utf-8")
                messagebox.showinfo("Export", self.LOCALES[self.ui_language].get('exported','Book exported.'))
                self.log(f'Exported "{b.title}" as {Path(path).name}')
            except Exception as e:
                messagebox.showerror("Error", f"Failed to export book:\n{e}")
    ...
    def set_ui_language(self, lang):
        ...
        try:
            ...
            try:
                self.btn_export.config(text=self.LOCALES[self.ui_language].get('export','Export Book (.txt)'))
            except Exception:
                pass
...
""".strip()
            ...
            # log error and raw AI output snippet for debugging
            try:
                if raw:
                    self.log(f"Raw AI output (truncated): {raw[:1000]}")
            except Exception:
                pass
    ...
    def extract_ideas_json(self, raw):
        raw = (raw or "").strip()
        # strip triple-backtick fences containing JSON
        m = re.search(r"```(?:json)?\s*([\s\S]*?)\s*```", raw, re.IGNORECASE)
        if m:
            raw = m.group(1).strip()

        # Helper: normalize smart quotes and remove trailing commas
        ...
        try:
            data = json.loads(raw)
            if isinstance(data, dict) and "ideas" in data and isinstance(data["ideas"], list) and len(data["ideas"]) == 5:
                return data["ideas"]
        except Exception:
            pass

        # try to find a balanced JSON object in the text (existing approach)
                ...
                for j in range(i + 1, len(raw)):
                    ...
                    if depth == 0:
                        ...
                        try:
                            ...
                            # try normalizing candidate and parsing again
                            try:
                                data = json.loads(_normalize(candidate))
                                ideas = data.get("ideas", [])
                                if isinstance(ideas, list) and len(ideas) == 5:
                                    return ideas
                            except Exception:
        ...
        # try to find an array anywhere by matching balanced brackets
        for i, ch in enumerate(raw):
            if ch == '[':
                depth = 1
                for j in range(i + 1, len(raw)):
                    ...
                    if depth == 0:
                        ...
                        try:
                            ...
                            pass
                        # try normalized candidate (smart quotes, trailing commas)
                        ...
                        try:
                            ...
                            # try converting single-quoted strings to double quotes
                            if norm.count('"') < norm.count("'"):
                                try:
                                    conv = re.sub(r"(?<!\\)'([^']*?)'(?!')", r'"\\1"', norm)
                                    arr = json.loads(conv)
                                    if isinstance(arr, list) and len(arr) == 5 and all(isinstance(x, str) for x in arr):
                                        return arr
                                except Exception:
                                    pass
                            # last fallback: ast.literal_eval on normalized string
                            try:
                                val = ast.literal_eval(norm)
                                if isinstance(val, list) and len(val) == 5 and all(isinstance(x, str) for x in val):
                                    return val
                            except Exception:
                                pass
                        break

        # try to locate a JSON-like "ideas" key and grab subsequent array
        m = re.search(r"\bideas\b\s*[:=]\s*", raw, re.IGNORECASE)
        if m:
            # find the first '[' after the match and attempt bracket matching
                ...
                for j in range(start + 1, len(raw)):
                    ...
                    if depth == 0:
                        ...
                        try:
                            ...
                            try:
                                arr = json.loads(_normalize(candidate))
                                if isinstance(arr, list) and len(arr) == 5:
                                    return arr
                            except Exception:
                                try:
                                    val = ast.literal_eval(_normalize(candidate))
        ...
        # fallback: split into paragraphs and try to select five substantial paragraphs
        paras = [p.strip() for p in re.split(r"\n\s*\n", raw) if p.strip()]
        if len(paras) >= 5:
            # choose paragraphs that look like sentences (heuristic: length > 40)
            chosen = [p for p in paras if len(p) > 40]
            if len(chosen) >= 5:
                return chosen[:5]

        # last attempt: numbered list extraction (lines starting with 1., 2., ...)
        nums = []
        for m in re.finditer(r"^\s*(?:\d+\.|\(|\-\s)\s*(.+)$", raw, re.M):
            txt = m.group(1).strip()
            if txt:
                nums.append(txt)
        if len(nums) >= 5:
        ...
        # give up and include a helpful debug snippet in the exception
        snippet = raw[:2000]
        self.log(f"Could not parse ideas JSON. Raw output snippet:\n{snippet}")
