...
function updateControl(poses) {
  ...
  const climbT = (opts.invClimb ? -1 : 1) * c / arms.length;
  ctrl.climb = smooth(ctrl.climb, climbT, 5, lastDt);
...
/* ---------- Opzioni (inversione assi) ---------- */
const opts = { invYaw: false, invClimb: false, invRoll: false };
function loadOpts() {
  try {
    const o = JSON.parse(localStorage.getItem("vola_opts_v1") || "{}");
    if (o && typeof o === "object") {
      opts.invYaw = !!o.invYaw;
      opts.invClimb = !!o.invClimb;
      opts.invRoll = !!o.invRoll;
    }
  } catch (e) { /* modalità privata */ }
  els.optYaw.checked = opts.invYaw;
  els.optClimb.checked = opts.invClimb;
  els.optRoll.checked = opts.invRoll;
}
function saveOpts() {
  try { localStorage.setItem("vola_opts_v1", JSON.stringify(opts)); } catch (e) {}
}
...
/* ---------- Opzioni: UI ---------- */
function bindOpt(input, key, label) {
  input.addEventListener("change", () => {
    opts[key] = input.checked;
    saveOpts();
    showToast("⚙️ " + label + (input.checked ? ": invertito" : ": normale"), 1300);
  });
}
els.optsBtn.addEventListener("click", () => {
  const hidden = els.optsPanel.classList.toggle("hidden");
  if (!hidden) showToast("Tocca gli interruttori per invertire un asse", 1500);
});
els.optsClose.addEventListener("click", () => els.optsPanel.classList.add("hidden"));
els.optsPanel.addEventListener("click", (e) => {
  if (e.target === els.optsPanel) els.optsPanel.classList.add("hidden");
});
bindOpt(els.optYaw, "invYaw", "Sinistra/destra");
bindOpt(els.optClimb, "invClimb", "Su/giù");
bindOpt(els.optRoll, "invRoll", "Virata");
loadOpts();
